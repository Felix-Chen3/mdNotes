...
6、静态导入包指定方法或变量，使得代码中可以不加类名直接使用该方法
...
（6）子类继承父类要么只能全部实现，否则就要继承为抽象类，但继承为抽象类的同时不影响实现其中的部分方法。
...
任何接口，如果只包含一个抽象方法，那么他就是一个函数接口。

那么对于函数式接口，就可以使用Lambda表达式来直接创建该接口的对象 
...
```java
1.受查异常 :编写代码的时候必须通过代码显示的声明并且抛出的异常
直接继承Exception除了RuntimeException外的所有子类异常和他们的子类
编写代码的时候要么继续上抛，要么直接捕获
2.非首查异常:编写代码的时候没有强制要求抛出的异常
在运行的时候虚拟机会抛出 RuntimeException 是那些可能在 Java 虚拟机正常运行期间抛出的异常的超类。 
...
public class Test {
	public static void main(String[] args) {
		...
		// 可以一直向上抛，但是在main之前必须有人去捕获它
		// 否则就会在运行的时候虚拟机发现没人捕获这个异常，这个就直接中断程序了
	}
}
...
class Two {
	public int get() throws ArithmeticException {
		One one = new One();
		int c = one.get(10, 0);// one.get有异常抛出
		// 如果发现被调用的方法有异常
		// 1.当场解决 在这里直接进行try catch
		// 2.自己解决不了，那么就继续向上抛
		return c;
	}
}
...
3、由于Java单继承的局限性，所以推荐使用实现runnable接口来实现多线程

4、实际上Thread类放入runnable接口的方式就是使用了静态代理，代理了一个真实的runna接口
...
```java
int num = Integer.valueOf(str);//返回引用类型
int a = Integer.parseInt("整数的字符串");//返回整数类型
...
（2）常量与变量 或 变量与变量：结果是堆（就是开辟了新的空间）
...
>         Date date = simpleDateFormat.parse("2019-33-98 22:99:22");
...
> ```
>
>  <img src="imgs/image-20200619220904021.png" alt="image-20200619220904021" style="zoom:67%;" /> 

### 11.5.1 JDK1.8之前

0、java.lang.System

>public static long cuurrentTimeMilies()
>
...
当前类自Comparable接口实现，并重写CompareTo逻辑

缺点是写死在类中，不能灵活改变

```java
//以age为要求进行CompareTO的重写
...
一个独立的类实现了Comparator的接口可以写非常灵活的逻辑，且可替换性强

一般在Arrays.sort(Object o,Comparator c)

通过更换c的实例来实现不同的排序逻辑调用

```java
ublic class Test3 {
	public static void main(String[] args) {
		Student[] stus = new Student[5];
		stus[0] = new Student(4, "x", 93);
		stus[1] = new Student(4, "x", 91);
		stus[2] = new Student(4, "a", 92);
		stus[3] = new Student(5, "r", 94);
		stus[4] = new Student(2, "w", 95);
	
		// 第二个参数就是Comparator比较器接口的实现类对象
		Comparator<Student> c = new CompareById();
		Arrays.sort(stus, c);// 传入一个新的比较规则覆盖原有的自然排序规则
...
![image-20200601102613447](imgs/image-20200601102613447.png)
...
　帮助封装。客户端可以更好的面向接口编程。
　解耦。客户端不需要知道实现类，只需要知道名称就能得到对应的实例对象
...
​	通过写新的实现类去实现原有的算法，不改变原有逻辑

#### 劣势

​	可能会有很多个实现类，每次传入的时候，都要更换引用接口指向的实际实现